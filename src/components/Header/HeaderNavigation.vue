...
<template>
  <div class="flex items-center gap-8 relative max-lg:gap-[24px]">
    <div v-for="(navItem, index) in headerNavs" :key="index" class="flex items-center relative">
      ...
      <div
        :class="[
          'absolute top-[calc(100%+22px)] pointer-events-none right-0 w-max flex flex-col items-start gap-2 p-3 rounded-[10px] border border-white/20 bg-dark-950/20 backdrop-blur-[10px] opacity-0  transition-all duration-300',
          {
...
<style scoped>
.link-hover-bg:hover {
  background: linear-gradient(to left, rgba(87, 193, 202, 0.1), rgba(158, 146, 245, 0.1));
}
</style>
